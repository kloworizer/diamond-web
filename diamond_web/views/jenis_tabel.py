...
class JenisTabelCreateView(LoginRequiredMixin, AdminP3DERequiredMixin, AjaxFormMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['form_action'] = reverse('jenis_tabel_create')
        return context
...
class JenisTabelUpdateView(LoginRequiredMixin, AdminP3DERequiredMixin, AjaxFormMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['form_action'] = reverse('jenis_tabel_update', args=[self.object.pk])
        return context
...
class JenisTabelDeleteView(SafeDeleteMixin, LoginRequiredMixin, AdminP3DERequiredMixin, DeleteView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['form_action'] = reverse('jenis_tabel_delete', args=[self.object.pk])
        return context
...
def jenis_tabel_data(request):
    ...
    for obj in qs_page:
        data.append({
            ...
            'actions': f"<button class='btn btn-sm btn-primary me-1' data-action='edit' data-url='{reverse('jenis_tabel_update', args=[obj.pk])}' title='Edit'><i class='ri-edit-line'></i></button>"
                       f"<button class='btn btn-sm btn-danger' data-action='delete' data-url='{reverse('jenis_tabel_delete', args=[obj.pk])}' title='Delete'><i class='ri-delete-bin-line'></i></button>"
        })
