...
def kategori_wilayah_data(request):
    ...
    for obj in qs_page:
        data.append({
            ...
            'actions': f"<button class='btn btn-sm btn-primary me-1' data-action='edit' data-url='{reverse('kategori_wilayah_update', args=[obj.pk])}' title='Edit'><i class='feather-edit-2'></i></button>"
                       f"<button class='btn btn-sm btn-danger' data-action='delete' data-url='{reverse('kategori_wilayah_delete', args=[obj.pk])}' title='Delete'><i class='feather-trash-2'></i></button>"
        })
