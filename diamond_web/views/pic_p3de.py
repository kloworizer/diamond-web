...
from ..forms.pic_p3de import PICP3DEForm
...
class PICP3DEListView(LoginRequiredMixin, AdminRequiredMixin, TemplateView):
    template_name = 'pic_p3de/list.html'
    ...
    def get(self, request, *args, **kwargs):
        ...
        if deleted and name:
            try:
                ...
                messages.success(request, f'PIC P3DE "{name}" deleted successfully.')
            except Exception:
                pass
        return super().get(request, *args, **kwargs)

class PICP3DECreateView(LoginRequiredMixin, AdminRequiredMixin, AjaxFormMixin, CreateView):
    model = PICP3DE
    form_class = PICP3DEForm
    template_name = 'pic_p3de/form.html'
    success_url = reverse_lazy('pic_p3de_list')
    success_message = 'PIC P3DE "{object}" created successfully.'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['form_action'] = reverse('pic_p3de_create')
        return context
...
class PICP3DEUpdateView(LoginRequiredMixin, AdminRequiredMixin, AjaxFormMixin, UpdateView):
    model = PICP3DE
    form_class = PICP3DEForm
    template_name = 'pic_p3de/form.html'
    success_url = reverse_lazy('pic_p3de_list')
    success_message = 'PIC P3DE "{object}" updated successfully.'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['form_action'] = reverse('pic_p3de_update', args=[self.object.pk])
        return context
...
class PICP3DEDeleteView(LoginRequiredMixin, AdminRequiredMixin, DeleteView):
    model = PICP3DE
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['form_action'] = reverse('pic_p3de_delete', args=[self.object.pk])
        return context
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({
                'success': True,
                'message': f'PIC P3DE "{name}" deleted successfully.'
            })
        messages.success(request, f'PIC P3DE "{name}" deleted successfully.')
...
def pic_p3de_data(request):
    ...
    qs = PICP3DE.objects.select_related('id_sub_jenis_data_ilap', 'id_user').all()
    ...
    for obj in qs_page:
        ...
        data.append({
            ...
            'actions': f"<button class='btn btn-sm btn-primary me-1' data-action='edit' data-url='{reverse('pic_p3de_update', args=[obj.pk])}' title='Edit'><i class='ri-edit-line'></i></button>"
                       f"<button class='btn btn-sm btn-danger' data-action='delete' data-url='{reverse('pic_p3de_delete', args=[obj.pk])}' title='Delete'><i class='ri-delete-bin-line'></i></button>"
        })
