...
from ..forms.pic_pmde import PICPMDEForm
...
class PICPMDEListView(LoginRequiredMixin, AdminRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if deleted and name:
            try:
                ...
                messages.success(request, f'PIC PMDE "{name}" deleted successfully.')
            except Exception:
                pass
        return super().get(request, *args, **kwargs)

class PICPMDECreateView(LoginRequiredMixin, AdminRequiredMixin, AjaxFormMixin, CreateView):
    model = PICPMDE
    form_class = PICPMDEForm
    template_name = 'pic_pmde/form.html'
    success_url = reverse_lazy('pic_pmde_list')
    success_message = 'PIC PMDE "{object}" created successfully.'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['form_action'] = reverse('pic_pmde_create')
        return context
...
class PICPMDEUpdateView(LoginRequiredMixin, AdminRequiredMixin, AjaxFormMixin, UpdateView):
    model = PICPMDE
    form_class = PICPMDEForm
    template_name = 'pic_pmde/form.html'
    success_url = reverse_lazy('pic_pmde_list')
    success_message = 'PIC PMDE "{object}" updated successfully.'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['form_action'] = reverse('pic_pmde_update', args=[self.object.pk])
        return context
...
class PICPMDEDeleteView(LoginRequiredMixin, AdminRequiredMixin, DeleteView):
    model = PICPMDE
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['form_action'] = reverse('pic_pmde_delete', args=[self.object.pk])
        return context
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return JsonResponse({
                'success': True,
                'message': f'PIC PMDE "{name}" deleted successfully.'
            })
        messages.success(request, f'PIC PMDE "{name}" deleted successfully.')
...
def pic_pmde_data(request):
    ...
    qs = PICPMDE.objects.select_related('id_sub_jenis_data_ilap', 'id_user').all()
    ...
    for obj in qs_page:
        ...
        data.append({
            ...
            'actions': f"<button class='btn btn-sm btn-primary me-1' data-action='edit' data-url='{reverse('pic_pmde_update', args=[obj.pk])}' title='Edit'><i class='ri-edit-line'></i></button>"
                       f"<button class='btn btn-sm btn-danger' data-action='delete' data-url='{reverse('pic_pmde_delete', args=[obj.pk])}' title='Delete'><i class='ri-delete-bin-line'></i></button>"
        })
