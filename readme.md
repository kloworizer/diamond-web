...
install pip package : 
di lokal : pip download -r requirements/base.txt -d ./packages & pip download -r requirements/dev.txt -d ./packages & pip download -r requirements/prod.txt -d ./packages & scp -r .\packages\* user@ip vm:/home/pajak/diamond-web/packages
di vm : pip install --no-index --find-links=./packages -r requirements/base.txt